...
### M0.1 Soveltamisala
Tämä opas koskee vihreän datakeskuksen (oma DC tai colocation) **sähkö–IT–jäähdytys–hukkalämpö** -ketjun **mitattavuutta, raportointia ja todennettavuutta**, jotta energiatehokkuus (PUE) ja muut vihreysvaatimukset eivät jää väitteiksi, vaan voidaan osoittaa mittausdatalla. [2][4][6][7][9]

### M0.2 Toteutusperiaate
PUE:n laskenta ja johtaminen edellyttävät vähintään **kokonaisenergian (E_total)** ja **IT-energian (E_IT)** erottelua. Lisäksi **UPS-häviöt** ja **jäähdytyksen energia** tulee pystyä osoittamaan osajärjestelmätasolla, jotta optimoinnin kohde on yksiselitteinen ja vaikutus todennettavissa. [2][6][7]
...
1) **Mittauspistekartta (Instrumentation & Metering Map)**  
   - mitä mitataan, millä tasolla (sisään tuleva energia, UPS, PDU/räkki, jäähdytys, lämpötoimitus) ja mittareiden yksilöinti. [6][7]  
...
3) **Datan omistajuus ja saatavuus (Data Access & Ownership)**  
   - kuka omistaa mittausdatan, miten data toimitetaan, ja miten audit trail säilyy. [2][4]  
...
**End-to-end todennus (“mittari → data → KPI”).** Tarkista vähintään:
- mittarien kalibrointi/luokka + aikaleimojen eheys
...
- Laske CO₂e ajanjaksolle: **CO₂e_total = E_total × EF_electricity** (raportointiperiaatteen mukainen päästökerroin). [9]
- Raportoi erikseen (suositus): CO₂e_total ja sen osatekijät (IT vs jäähdytys vs häviöt), jos erittely on mitattavissa. [2][4][7]
...
- Raportoi lämpö **erillisenä hyötymittarina**, ei automaattisena vähennyksenä CO₂e:stä, ellei laskentasääntöä ole eksplisiittisesti sovittu ja auditoitavissa. [9]

---

## M6. Optimointimenetelmä (mistä CO₂e vähenee käytännössä)

Menetelmä kytkeytyy toimintaketjuun: sähkö → IT → verkko → jäähdytys → lämpö. [1][6][7]
...
- Setpointit, ilmavirrat/containment, vapaajäähdytyksen hyödyntäminen. [4][7]
...
### M6.5 Reaaliaikainen valvonta ja AI/DA-ohjaus (todennettava toteutus)
AI/DA-ohjaus on perusteltua vasta, kun mittausketju ja todennus ovat kunnossa (M1–M4). Tavoite on muuttaa mittausdata **ohjaukseksi**, jonka vaikutus voidaan osoittaa “ennen–jälkeen” -asetelmassa. [2][4]

**Mitä mitataan reaaliajassa (minimi):**
- E_total, E_IT, PUE (sekä mielellään UPS-häviöt ja E_cooling), ja olosuhteet (lämpötila/ΔT) vähintään sali- tai vyöhyketasolla. [4][6][7]

**Mitä AI optimoi (tyypilliset ohjausmuuttujat):**
- IT: kuorman sijoittelu/konsolidointi ja tehorajat (SLA:n puitteissa). [1][2]
- Jäähdytys: setpointit, puhaltimet/pumput, vapaajäähdytyksen hyödyntäminen ja ohjauslogiikan parametrit. [4][7]
- (Valinnainen) verkko: energiatilat liikenteen mukaan, jos mitattavissa. [8]

**Rajoitteet, joita ei saa rikkoa (pakolliset):**
- SLA ja käytettävyys, lämpötilarajat, redundanssi-/vikasietoisuusvaatimukset sekä turvallisuusvaatimukset.

**Miten vaikutus todennetaan (audit-kelpoisesti):**
- jokaiselle ohjausmuutokselle baseline, muutos, mittausjakso, vaikutus KPI:hin ja riskit/SLA (M4.3)
- laskentasäännöt ja malliversiot versionhallitaan; KPI:n laskenta ei muutu “hiljaisesti” (M4.2). [2][4]

---

## M7. Mitä asiakkaana saat ulos (API/raportti) – vähimmäisvaatimus

M7 toimii käytännössä **“sustainability data pack”** -liitteenä, jonka colocation-asiakas voi vaatia sopimukseen. Datan tulee olla saatavilla sekä **ihmisluettavana raporttina** että **koneellisesti** (CSV/JSON/API). Lisäksi toimitukseen sisällytetään datan laadun tunnusluvut (esim. puuttuvan datan osuus ja aikaleimojen eheys), jotta raportointi on todennettavaa. [2][4]

> **Vaatimus:** KPI-laskentasäännöt ja niiden muutokset (versiot) on dokumentoitava; laskenta ei saa muuttua ilman jäljitettävää muutospolkua (M4.2). [2][4]
